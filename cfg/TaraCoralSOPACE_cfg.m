...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cfg.meta.investigators = 'Alison_Chase,Guillaume_Bourdin,Nils_Haentjens';
cfg.meta.affiliations = 'University_of_Washington';
cfg.meta.emails = 'alichase@uw.edu';
cfg.meta.experiment = 'TaraCoralSOPACE';
cfg.meta.cruise = 'TaraCoralSOPACE';
cfg.meta.station = 'NA';
...
PATH_ROOT = '/Volumes/Chase-SSD1/TaraCoral-SOPACE';
...
%%% NMEA %%%
model = 'GPSSC701'; % GPSSC701Tara GPS32Tara GPSCOMPASSAT
SN = '';
...
cfg.instruments.FLOW.logger = 'Inlinino_base'; % FlowControl Inlinino_base
...
cfg.instruments.FLOW.path = struct('raw', fullfile(PATH_ROOT, 'raw', [model SN]),...
                                  ...
                                  'prod', fullfile(PATH_ROOT, 'prod'),...
                                  'ui', fullfile(PATH_ROOT, 'ui', [model SN]));
cfg.instruments.FLOW.view = struct('varname', 'swt1','swt_variable','swt1','spd_variable','spd1'); % spd1 spd2
...
cfg.instruments.([model SN]).device_file = fullfile(PATH_ROOT, 'DeviceFiles', 'ACS279_20231006.dev');
cfg.instruments.([model SN]).path = struct('raw', fullfile(PATH_ROOT, 'raw', [model SN]),...
...
% cfg.instruments.([model SN]).AC_source = true;
% cfg.instruments.([model SN]).logger = 'InlininoACScsv';
% cfg.instruments.([model SN]).device_file = fullfile(PATH_ROOT, 'DeviceFiles', 'ACS111_20231006.dev');
% cfg.instruments.([model SN]).path = struct('raw', fullfile(PATH_ROOT, 'raw', [model SN]),...
%                                     'di', fullfile(PATH_ROOT, 'raw', [model SN], 'DI'),...
%                                     'wk', fullfile(PATH_ROOT, 'wk', [model SN]),...
%                                     'prod', fullfile(PATH_ROOT, 'prod'),...
...
cfg.instruments.([model SN]).analog_channel = 'C2_SUVF6266';
...
cfg.instruments.([model SN]).sn = SN;
cfg.instruments.([model SN]).logger = 'Inlinino_base';
cfg.instruments.([model SN]).scale = 7.013E-04; % Volts/(uE/m²/sec)
cfg.instruments.([model SN]).dark = 10.323E-03;
cfg.instruments.([model SN]).path = struct('raw', fullfile(PATH_ROOT, 'raw', [model SN]),...
                                  'wk', fullfile(PATH_ROOT, 'wk', [model SN]),...
                                  'prod', fullfile(PATH_ROOT, 'prod'),...
                                  'ui', fullfile(PATH_ROOT, 'ui', [model SN]));
cfg.instruments.([model SN]).view = struct('varname', 'PAR');
...
for i = 1:length(cfg.process.instruments2run)
  ...
  elseif any(contains(lower(cfg.process.instruments2run{i}), 'lisst100x'))
    cfg.process.split.buffer.(cfg.process.instruments2run{i}) = seconds([180, 100]); % [540, 360] for LISST
  ...
  elseif any(contains(lower(cfg.process.instruments2run{i}), {'lissttau','lisst-tau'}))
    cfg.process.split.buffer.(cfg.process.instruments2run{i}) = seconds([180, 100]); % [180, 60] for LISST-Tau
  else
