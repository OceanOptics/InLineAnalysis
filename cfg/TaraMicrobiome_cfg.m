...
cfg.instruments.FLOW.view = struct('varname', 'swt','swt_variable','swt','spd_variable','spd2'); % spd1 spd2
...
cfg.instruments.(['BB3' SN]).k_exp = 0.01635;
...
if contains(interface, 'ADU100') || contains(interface, 'DataQ')
  cfg.instruments.(['WSCD' SN]).ila_prefix = interface;
  raw_path = fullfile(PATH_ROOT, 'raw', interface);
else
  cfg.instruments.(['WSCD' SN]).ila_prefix = ['WSCD' SN];
  raw_path = ['WSCD' SN];
end
cfg.instruments.(['WSCD' SN]).path = struct('raw',  raw_path,...
                                  'di',  raw_path,...
...
cfg.instruments.([model SN]).zsc = fullfile(PATH_ROOT, 'DeviceFiles', 'LISST100X1183_20180119_factory_zsc.asc');
cfg.instruments.([model SN]).dcal = fullfile(PATH_ROOT, 'DeviceFiles', 'LISST100X1183_20180119_Ringarea.asc');
cfg.instruments.([model SN]).vcc = fullfile(PATH_ROOT, 'DeviceFiles', 'LISST100X1183_20180119_InstrumentData.txt');
...
% cfg.process.sync.delay.ACS57 = seconds(0);
% cfg.process.sync.delay.ACS348 = seconds(0);
% cfg.process.sync.delay.BB31502 = seconds(0);
% cfg.process.sync.delay.WSCD859 = seconds(0);
% cfg.process.sync.delay.NMEA = seconds(0);
% cfg.process.sync.delay.HyperBB8005 = seconds(0);
% cfg.process.sync.delay.LISST1183 = seconds(0);
% cfg.process.sync.delay.LISSTTau1002G = seconds(0);
% cfg.process.sync.delay.SUVF6244 = seconds(0);
cfg.process.sync.delay.LISST200X2002 = seconds(-90);
...
% cfg.process.split.buffer.SUVF6244 = seconds([240, 100]); % [660, 100]
