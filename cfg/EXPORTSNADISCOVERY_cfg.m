...
cfg.meta.cruise = 'EXPORTSNADISCOVERY';
cfg.meta.station = 'NA';
...
PATH_ROOT = '/Volumes/Chase-SSD1/EXPORTSNADISCOVERY';
...
cfg.instruments.([model SN]).view = struct('varname', 'sst');% why is this t2? isn't it really tsg or ctd?
cfg.instruments.([model SN]).temperature_variable = 'sst';%changed from t2 to sst in data file header
cfg.instruments.([model SN]).salinity_variable = 'sss';%changed from s to sss in data file header
...
cfg.instruments.FLOW.logger = 'FlowControl'; % FlowControl Inlinino_base
...
cfg.instruments.FLOW.path = struct('raw', fullfile(PATH_ROOT, 'raw', 'FlowControl'),...
                                  ...
                                  'prod', fullfile(PATH_ROOT, 'prod'),...
                                  'ui', fullfile(PATH_ROOT, 'ui', 'FlowControl'));
cfg.instruments.FLOW.view = struct('varname', 'swt','swt_variable','swt','spd_variable','spd1'); % spd1 spd2
...
cfg.process.split.buffer.ACS298 = seconds([480, 240]);
