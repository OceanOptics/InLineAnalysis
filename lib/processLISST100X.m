function [p, pbin_size, g, gbin_size] = processLISST100X(param, tot, filt, di, fth, fth_constants, di_method, days2run)
  % processLISST100X process LISST100X from flow through system with both
  ...
  fprintf('VCC = %i\n', param.vcc)
  fprintf('inversion = %s\n', param.inversion)
  fprintf('dcal = %s\n', cell2mat(join(strip(cellstr(num2str(param.dcal'))'), '   ')))
  fprintf('theta = %s\n', cell2mat(join(strip(cellstr(num2str(param.theta'))'), '   ')))
  ...
  if ~isempty(filt)
    if exist('fth', 'var')
      ...
      fth_interp.swt = interp1(fth.qc.tsw.dt, fth.qc.tsw.(fth.view.swt_variable), fth_interp.dt, 'previous');%, 'linear', 'extrap');
      ...
      filt_avg.bincount = NaN(size(filt_avg,1), 1);
      for i=1:size(sel_start, 1)
        sel_filt = fth_interp.dt(sel_start(i)) <= filt.dt & filt.dt <= fth_interp.dt(sel_end(i));
        foo = filt(sel_filt,:);
        foo.laser_reference = repmat(foo.laser_reference, 1, size(filt.beta, 2));
        foo.(LASER_POWER_VAR) = repmat(foo.(LASER_POWER_VAR), 1, size(filt.beta, 2));
        if sum(sel_filt) == 1
          filt_avg.dt(i) = foo.dt;
          filt_avg.beta(i,:) = foo.beta;
          filt_avg.beta_avg_sd(i,:) = foo.beta_avg_sd;
          filt_avg.laser_reference(i) = foo.laser_reference(1);
          filt_avg.(LASER_POWER_VAR)(i,:) = foo.(LASER_POWER_VAR)(1);
        elseif sum(sel_filt) > 1
          ...
          filt_avg.laser_reference(i) = mean(foo.laser_reference(:), 'omitnan');
          filt_avg.(LASER_POWER_VAR)(i) = mean(foo.(LASER_POWER_VAR)(:), 'omitnan');
        end
      end
    ...
    if exist('visFlag', 'file') && exist('fth', 'var')
      ...
      legend('Flow rate','Filtered interpolated','Binned total','Filtered median',...
        'AutoUpdate','off', 'FontSize', 12)
      guiSelectOnTimeSeries(fh);
    elseif exist('visFlag', 'file')
      fh = visFlag([], filt_interp(filt_interp_id, :), tot(tot_id, :), [], filt_avg(filt_avg_id, :), [], 'beta', round(size(tot.beta, 2)/2), [], []);
      title('Check filter event interpolation, press q to continue', 'FontSize', 14)
      legend('Filtered interpolated','Binned total','Filtered median',...
        'AutoUpdate','off', 'FontSize', 12)
      guiSelectOnTimeSeries(fh);
    end
  else
  ...
  % % %   case 'spherical'
  % % %     non_spherical_inversion = 0;
  % % %   case 'non-spherical'
  % % %     non_spherical_inversion = 1;
  % % %   otherwise
  % % %     error('Unknown inversion type. Only "spherical" or "non-spherical" supported');
  % % % end
  ...
  % % %   p.VD(i,:) = invert_2014b(p.betap(i,:),2,0,non_spherical_inversion,0,0,0) ./ param.vcc ...
  % % %               .* zsc_laser_reference ./ (tot.laser_reference(i).*ones(1,32)); % unit of microl/l, or 10^-6 micron^3/micron^3
  % % %   p.VD_sd(i,:) = invert_2014b(p.betap_sd(i,:),2,0,non_spherical_inversion,0,0,0) ./ param.vcc ...
...
function [prod, bin_size] = compute_product_LISST100X(tot, filt_interp, param, phi, path_length, LASER_POWER_VAR)
  ...
  % Correct particulate scattering for detector area and responsivness
  prod.betap = param.dcal .* prod.betap;
  prod.betap_sd = param.dcal .* prod.betap_sd;
  ...
  if strcmpi(param.inversion, 'spherical')
    InvType = 0;
  elseif strcmpi(param.inversion, 'non-spherical') || strcmpi(param.inversion, 'irregular')
    InvType = 1;
  else
    error('Unknown LISST-100X Inversion Type')
  end

  % prod.invert = NaN(size(prod.betap));
  prod.VD = NaN(size(prod.betap));
  prod.VD_sd = NaN(size(prod.betap));

  fprintf('VSF computation ... \n')
  for i=progress(1:size(prod,1))
    % Use factory zsc taken at the same time as the VCC for the inversion.
    % Need betap in counts (NOT Scientific Units)
    prod.VD(i,:) = invert_2014b(prod.betap(i,:),2,0,InvType,0,0,0) ./ param.vcc ...
                .* filt_interp.laser_reference(i) ./ (tot.laser_reference(i).*ones(1,32)); % unit of \muL/L, or 10^-6 micron^3/micron^3
    prod.VD_sd(i,:) = invert_2014b(prod.betap_sd(i,:),2,0,InvType,0,0,0) ./ param.vcc ...
                .* filt_interp.laser_reference(i) ./ (tot.laser_reference(i).*ones(1,32)); % standard deviation (same units)
  end
  % Convert betap from counts to scientific units (1/m)
  ...
  % Convert to number distribution
  fprintf('PSD computation ... ')
  prod.PSD = (prod.VD ./ (pi*d.^3/6) ./ bs) * 10^6; % units: # / \mum^3 / m
  fprintf('done\n')
  fprintf('VSD computation ... ')
  prod.VSD = (prod.VD ./ bs) * 10^6;                % units: ppm / m 
  fprintf('done\n')
  prod.bincount = tot.beta_avg_n;

  % Perform simple QC
  prod(any(prod.PSD < 0,2),:) = [];
  
  % add variable units and lisst parameters
  prod.Properties.VariableUnits = {'','1/m','1/m/Sr','1/m/Sr','uL/L','uL/L','nb/mL/micron', 'ppm/m', 'none'};
  prod.Properties.UserData = param;
  
  param_fnames = fieldnames(param);
  for p = param_fnames'
    prod = addprop(prod, p{:}, 'table');
    prod.Properties.CustomProperties.(p{:}) = param.(p{:});
  end
end