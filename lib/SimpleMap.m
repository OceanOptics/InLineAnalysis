...
function figh = SimpleMap(data, station_info, colorbarlabel, cmap_limit, bubble_data, bubble_label, print_station_name, text_col, biogeography)
  ...
  %       - optional variable: dt <Nx1 datetime> date/time of sampling to be displayed on data tip
  ...
  if license('test', 'MAP_Toolbox')
    ...
    if biogeography
      ax1.Visible = 'off';
      ax1 = axesm('robinson', 'MapLatLimit', latlim, 'MapLonLimit', lonlim, 'mlinevisible', 'off', ...
    ...
    if ~isempty(bubble_data) && size(data,2) == 3 && all(~isnan(data),'all') && max(data(:)) <= 1
      ...
      if ~isempty(bubble_data) % if bubble
        ...
        if size(unique(data(~all(isnan(data),2))), 1) == 1
          % data(data == 0) = NaN;
          scm(2) = scatterm(ax1, station_info.lat(~all(isnan(data),2)), ...
            station_info.lon(~all(isnan(data),2)), 80, ...
            'filled', 'MarkerFaceAlpha', 1, 'MarkerEdgeAlpha', 1, ...
            'MarkerEdgeColor', edgcol, 'Marker', 'o');
...
%     meadj = 10.^(log(nthroot((me - 10) / 50, 2.5)) * max(log10(bubble_data(:))));
    ...
    % x_spac = 0.095;
    % scatter(x,y, me/max(me)*400+100,'k','Marker','o'); % TODO dynamic for multiple marker type (e.g. for 4 different marker)
  ...
  % display ID, dt, lat, and lon on map tip
  if strcmp(figh.Type, 'figure')
    set(datacursormode(figh), 'UpdateFcn', @displayMapCoordinates);
  end
  function txt = displayMapCoordinates(~, event_obj)
    indx = get(event_obj,'DataIndex');
    if strcmp(station_info.Properties.VariableNames, 'dt')
      txt = sprintf('ID: %s\ndt: %s\nlat: %.4f\nlon: %.4f', station_info.station(indx), ...
        station_info.dt(indx), station_info.lat(indx), station_info.lon(indx));
    else
      txt = sprintf('ID: %s\nlat: %.4f\nlon: %.4f', station_info.station(indx), ...
        station_info.lat(indx), station_info.lon(indx));
    end
  end
end
