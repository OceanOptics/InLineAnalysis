...
if license('test','MAP_Toolbox')   
    ...
    if ~varargin{1} && size(data,2) == 3 && all(~isnan(data),'all') && max(data(:)) <= 1
        scm(2) = scatterm(station_info.lat, ...
            ...
            data, ...
            'filled','MarkerFaceAlpha',0.2,'MarkerEdgeAlpha',0.3,...
            'MarkerEdgeColor', edgcol, 'Marker', 'o');
            scm(2).Children.MarkerFaceAlpha = .3; %.75
            scm(2).Children.MarkerEdgeAlpha = .05; %.15
            scm(2).Children.LineWidth = 0.001;
    else
        % plot station without any count in grey
        scm(1) = scatterm(station_info.lat(all(isnan(data),2) | data ==0), ...
            station_info.lon(all(isnan(data),2) | data ==0), 20, [0.65 0.65 0.65], ...
            'filled','MarkerFaceAlpha',0.2,'MarkerEdgeAlpha',0.3, ...
        ...
        if varargin{1} && ~isempty(varargin{2}) % if bubble
            ...
            scm(2) = scatterm(station_info.lat(~all(isnan(data),2)), ...
                ...
                'filled','MarkerFaceAlpha',0.6,'MarkerEdgeAlpha',0.9,...
                'MarkerEdgeColor',edgcol, 'Marker', 'o');
            scm(2).Children.MarkerFaceAlpha = .3; %.75
            scm(2).Children.MarkerEdgeAlpha = .05; %.15
            scm(2).Children.LineWidth = 0.001;
