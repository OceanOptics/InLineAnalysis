function [p, g, FiltStat] = processHBB(param, tot, filt_qc, filt_raw, filt_bad, di, ...
  ...
  if strcmp(filt_method, '25percentil')
    ...
    if isempty(filt_qc)
      error('No HBB qc data loaded, when "filt_method == 25percentil", HBB qc data level should be loaded')
    end
  ...
  elseif strcmp(filt_method, 'exponential_fit')
    ...
    if isempty(filt_raw)
      error('No HBB raw data loaded, when "filt_method == exponential_fit", HBB raw data level should be loaded')
    end
    ...
    if isempty(ac_tbl_name)
      ...
      if any(strcmp(ac_tbl_name, 'p'))
        if isempty(AC.prod.p)
          warning('No ap prod data loaded')
          ac_p = [];
        else
          ac_p = AC.prod.p;
      ...
      if any(strcmp(ac_tbl_name, 'g'))
        if isempty(AC.prod.g)
          warning('No ag prod data loaded')
          ac_g = [];
        else
          ac_g = AC.prod.g;
  ...
  fdom_data = [];
  if ~isempty(ac_p) && isempty(ac_g) && ~isempty(param.fdom_ag_parameters)
    if ~isempty(CDOM)
      ...
      warning('fdom_ag_parameters loaded but no FDOM prod data loaded')
    end
  elseif ~isempty(ac_p) && isempty(ac_g) && ~any(strcmp(ac_p.Properties.VariableNames, 'ag_modelled'))
    warning('No fdom_ag_parameters and FDOM prod data loaded')
  end
  ...
  if ~isempty(tsg_data)
    ...
    % end
    % Get beta seawater from Zhang et al. 2019
    [tot_beta_sw, ~] = betasw_HZP2019(param.lambda, param.theta, filt_interp.t, filt_interp.s, 1);
  ...
  if ~isempty(tsg_data)
    ...
    % end
    % Get beta seawater from Zhang et al. 2019
    [filt_beta_sw, ~] = betasw_HZP2019(param.lambda, param.theta, filt_avg.t, filt_avg.s, 1);
    % substract beta seawater from filt_avg event
    filt_avg.beta(~filt_ts_nan, :) = filt_avg.beta(~filt_ts_nan, :) - filt_beta_sw(~filt_ts_nan, :);
    % flag when T/S not available in filt_avg events
  ...
  if exist('visFlag', 'file') && exist('FTH', 'var')
    ...
    legend('Flow rate','Filtered binned interpolated', 'Total binned', 'Filtered binned',...
      'AutoUpdate','off', 'FontSize', 12)
    guiSelectOnTimeSeries(fh);
  elseif exist('visFlag', 'file')
    fh = visFlag([], filt_interp(filt_interp_id, :), tot(tot_id, :), [], filt_avg(filt_avg_id, :), [], 'beta', round(size(tot.beta, 2)/2), [], []);
    title('Check filter event interpolation, press q to continue', 'FontSize', 14)
    legend('Filtered binned interpolated', 'Total binned', 'Filtered binned',...
      'AutoUpdate','off', 'FontSize', 12)
    guiSelectOnTimeSeries(fh);
  end
  ...
  if strcmpi(AC.model, 'lisst100x')
    instrument_types.c_type = [AC.model '-' AC.type];
  else
    instrument_types.c_type = AC.model;
  end
  ...
  elseif strcmpi(instrument_types.c_type, 'lisst100x-b')
    instrument_types.acc_ang_c = 0.0263; % Emmanuel found 0.0269
  elseif strcmpi(instrument_types.c_type, 'lisst100x-c')
    instrument_types.acc_ang_c = 0.0132;
  elseif strcmpi(instrument_types.c_type, 'lisst100-floc')
    instrument_types.acc_ang_c = 0.0043; % Emmanuel measured 0.006
  elseif strcmpi(instrument_types.c_type, 'lisst200x')
    instrument_types.acc_ang_c = 0.08;
  else
    error('c instrument type unknown')
  end
  instrument_types.ang_a = 180;
  [p.betap, p.bbp, flags] = betap_Kcorrection(tot, filt_interp, param.lambda, param.k_exp, ...
    X_p, ac_p, ac_g, AC.lambda_a, AC.lambda_a, instrument_types, param.fdom_ag_parameters);
  ...
  % Propagate error
  %   Note: Error is not propagated through attenuation correction
  ...
  % Compute dissolved product for quality check: should be around 0
  if nargout > 1 && any(~isempty(di) | strcmp(di_method,'SW_scattering')) && ~isempty(tsg_data)
    ...
    % end
    % Get beta seawater from Zhang et al. 2019
    [filt_beta_sw, ~] = betasw_HZP2019(param.lambda, param.theta, filt_qc.t, filt_qc.s, 1);
    ...
    if ~isempty(di)
      ...
      % for j = 1:size(filt_qc,1)
      %   beta_tdiw(j, :) = betasw_ZHH2009(param.lambda, filt_qc.WaterTemp(j), param.theta, 0);
      % end
      % Get beta seawater from Zhang et al. 2019
      [beta_tdiw, ~] = betasw_HZP2019(param.lambda, param.theta, filt_qc.WaterTemp, 0, 1);
    ...
    switch di_method
      case 'interpolate'
        ...
        % Propagate error
        %   Note: Error is not propagated through Zhang betasw_HZP2019
        g.betag_sd = sqrt(filt_qc.beta_avg_sd.^2 + di_pp.beta_avg_sd.^2);
      case 'constant'
        % Average all given DI samples
        %     + recommended if no drift is observed with sensor
        di_pp = table(NaT, 'VariableNames', {'dt'});
