...
%% 'NMEA','FLOW','SBE455462270286','ACS279','HyperBB8005','SUVF6266','LISST100X1183'
if ~isfile(fullfile(ila.instrument.FLOW.path.prod, 'TaraCoralSOPACE_InLine_GPS_20251219_20260404_Product_v20260528.mat'))
  ila.cfg.days2run = datetime(2025,12,16):datetime(2026,4,9);
  ...
  latlon(latlon.lat > 9 & latlon.lat < 10 & latlon.lon < -30 & latlon.lon > -40, :) = [];
  ...
  SimpleMap(NaN(size(latlon.dt)), latlon(:,1:3), 'TaraCoral-SOPACE track')
  close Figure 1
  ...
  load(fullfile(ila.instrument.FLOW.path.prod, 'TaraCoralSOPACE_InLine_GPS_20251219_20260404_Product_v20260528.mat'))
end

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TSG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ~isfile(fullfile(ila.instrument.FLOW.path.prod, 'TaraCoralSOPACE_InLine_TSG_20251225_20260404_Product_v20260529.mat'))
  ila.cfg.days2run = datetime(2025,12,16):datetime(2026,4,9);
  ila.cfg.instruments2run = {'SBE455462270286'};
  
  % populate ila.instrument
  ila.Read('prod');
  
  % extract TSG data from ila
  tsg_temp = round_timestamp(ila.instrument.('SBE455462270286').prod.a);
  ...
  tsg = removevars(tsg, {'t2','t2_avg_sd','t2_avg_n','t1_avg_n','c1_avg_n','s_avg_n'});
  tsg = renamevars(tsg,{'t1','t1_avg_sd','c1','c1_avg_sd','s','s_avg_sd','sv_avg_n'}, ...
    {'sst','sst_avg_sd','cond','cond_avg_sd','sss','sss_avg_sd','avg_n'});
  
  tsg.Properties.VariableUnits = {'','degrees','degrees','degreesC','degreesC',...
    'S/m','S/m','PSU','PSU','m/sec','m/sec','none'};
  ...
  load(fullfile(ila.instrument.FLOW.path.prod, 'TaraCoralSOPACE_InLine_TSG_20251225_20260404_Product_v20260529.mat'))
end

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% SUVF6266 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ~isfile(fullfile(ila.instrument.FLOW.path.prod, 'TaraCoralSOPACE_InLine_SUVF6266_20251220_20260404_Product_v20260529.mat'))
  ...
  replace_consecutive_nan = 3*60; % 3h
  suvf = merge_timeseries(suvf_temp, tsg, {'sst', 'sss'});
  ...
  % % export product to SeaBASS format
  % ila.meta.documents = sprintf('%s_%s_ProcessingReport_v%s.pdf', cruise, ila.cfg.instruments2run{:}, datetime(2026,5,31, 'Format', 'yyyyMMdd'));
  ...
  load(fullfile(ila.instrument.FLOW.path.prod, 'TaraCoralSOPACE_InLine_SUVF6266_20251220_20260404_Product_v20260529.mat'))
end

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% HBB %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
list_leg = table();
list_leg.dt = [...
  datetime(2025,12,16) datetime(2026,4,9);
  ];
list_leg.experiment = {'PVST_SOPACE';};
list_leg.cruise = {'TaraCoralSOPACE';};
...
for i=1:size(list_leg, 1)
  ...
  hbb_temp = merge_timeseries(hbb_temp, tsg, {'sst','sss'});
...
list_leg.experiment = {'PVST_SOPACE';};
list_leg.cruise = {'TaraCoralSOPACE';};
list_leg.devfile = {'ACS279_20231006.dev';};
list_leg.instru = {'ACS279';};

data_AC = struct('spectral', [], 'product', []);
acs_prod = [];
% acs_prod_diw = [];
...
for i=1:size(list_leg, 1)
  ...
  ACp = merge_timeseries(ACp, tsg, {'sst','sss'});
  ...
  % split into spectra and products tables
  id_ancil = logical(sum(categorical(ACp.Properties.VariableNames) == {'dt','lat','lon','t','s','fdom','ap_n'}'));
  id_spectral = logical(sum(categorical(ACp.Properties.VariableNames) == {'ap','ap_unc','cp','cp_unc','ag_modelled','cg_modelled'}'));
  data_AC.spectral.(ref) = ACp(:, id_ancil | id_spectral);
  % rename variables for SeaBASS
  data_AC.spectral.(ref) = renamevars(data_AC.spectral.(ref), {'fdom','ap_n','ag_modelled','cg_modelled'}, {'cdomf','bincount','ag','cg'});
  data_AC.spectral.(ref) = movevars(data_AC.spectral.(ref), {'ag','cg'}, 'Before','bincount');

  % export particulate to SeaBASS format
  filename_part = sprintf('%s_InLine_%s_Spectral_%s_%s_v%s.sb', list_leg.cruise{i}, list_leg.instru{i}, ...
    datetime(min(data_AC.spectral.(ref).dt), 'Format', 'yyyyMMdd'),...
    datetime(max(data_AC.spectral.(ref).dt), 'Format', 'yyyyMMdd'),...
    datetime('today', 'Format', 'yyyyMMdd'));
  ila.meta.documents = sprintf('%s_ACS_ProcessingReport_v%s.pdf', cruise, datetime(2026,5,31, 'Format', 'yyyyMMdd'));
  ...
  data_AC.product.(ref) = ACp(:, id_ancil| ~id_spectral);
  % rename variables for SeaBASS
  ...
  filename_prod = strrep(filename_part, '_Spectral_', '_ProductsFull_');
  ila.meta.documents = sprintf('%s_ACS_ProcessingReport_v%s.pdf', cruise, datetime(2026,5,31, 'Format', 'yyyyMMdd'));
  ...
  if isfield(ila.instrument.(list_leg.instru{i}).prod, 'g')
    if ~isempty(ila.instrument.(list_leg.instru{i}).prod.g)
      ...
      % build AC table: merge lat, lon, sst, sss, and fdom
      replace_consecutive_nan = 3*60; % 3h
      ACg = merge_timeseries(ACg, tsg, {'sst','sss'});
      ...
      filename_diw = strrep(filename_part, '_Spectral_', '_Dissolved_MilliQ_');
      ila.meta.documents = sprintf('%s_ACS_ProcessingReport_v%s.pdf', cruise, datetime(2026,5,31, 'Format', 'yyyyMMdd'));
      ila.meta.calibration_files = list_leg.devfile{i};
