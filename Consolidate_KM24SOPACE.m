...
%% 'NMEA','FLOW','SBE3845KM24','ACS91','ACS111','HyperBB8005','SUVF6244','LISST100X1183','WS3S1081P','ALFA'
if ~isfile(fullfile(ila.instrument.FLOW.path.prod, 'KM24SOPACE_InLine_GPS_20241024_20241216_Product_v20250212.mat'))
  ...
  latlon = removevars(latlon, {'lat_avg_sd','lat_avg_n','lon_avg_sd','lon_avg_n',...
    'altitude','altitude_avg_sd','altitude_avg_n','gps_qual','gps_qual_avg_sd','gps_qual_avg_n',...
    'num_sats','num_sats_avg_sd','num_sats_avg_n','horizontal_dil','horizontal_dil_avg_sd','horizontal_dil_avg_n',...
    'true_course','true_course_avg_sd','true_course_avg_n'});
  latlon = round_timestamp(latlon);

  SimpleMap(NaN(size(latlon.dt)), latlon(:,1:3), 'KM24-SOPACE track')
  close Figure 1

  filename = sprintf('%s_InLine_%s_%s_%s_Product_v%s.mat', cruise, 'GPS', ...
  ...
  load(fullfile(ila.instrument.FLOW.path.prod, 'KM24SOPACE_InLine_GPS_20241024_20241216_Product_v20250212.mat'))
end

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TSG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ~isfile(fullfile(ila.instrument.FLOW.path.prod, 'KM24SOPACE_InLine_TSG_20241024_20241216_Product_v20250212.mat'))
  ila.cfg.days2run = datetime(2024,10,23):datetime(2024,12,16);
  ila.cfg.instruments2run = {'SBE3845KM24'};
  
  % populate ila.instrument
  ila.Read('prod');
  
  % extract TSG data from ila
  tsg_temp = round_timestamp(ila.instrument.('SBE3845KM24').prod.a);
  ...
  tsg = removevars(tsg, {'year','year_avg_sd','year_avg_n','jday',...
    'jday_avg_sd','jday_avg_n','hour','hour_avg_sd','hour_avg_n','min',...
    'min_avg_sd','min_avg_n','sec','sec_avg_sd','sec_avg_n','msec',...
    'msec_avg_sd','msec_avg_n','tcal','tcal_avg_sd','tcal_avg_n',...
    'c_avg_n','sss_avg_n','sst_avg_n'});
  
  tsg = renamevars(tsg,{'c','c_avg_sd','sound_velocity','sound_velocity_avg_sd','sound_velocity_avg_n'}, ...
    {'cond','cond_avg_sd','sv','sv_avg_sd','avg_n'});
  
  tsg.Properties.VariableUnits = {'', 'degrees', 'degrees', 'S/m', 'S/m', ...
    'PSU', 'PSU', 'degreesC', 'degreesC', 'm/sec', 'm/sec', 'none'};
  % tsg.Properties.VariableDescriptions = {'', '%.4f', '%.4f', '%.4f', '%.4f', ...
  %   '%.4f', '%.4f', '%.4f', '%.4f', '%.2f', '%.2f', '%.2f'};
  tsg.Properties.VariableDescriptions = {'', '%e', '%e', '%e', '%e', ...
    '%e', '%e', '%e', '%e', '%e', '%e', '%e'};
  ...
  load(fullfile(ila.instrument.FLOW.path.prod, 'KM24SOPACE_InLine_TSG_20241024_20241216_Product_v20250212.mat'))
end
...
% wscd.Properties.VariableDescriptions = {'', '%e', '%e', '%e', '%e', '%e', '%e', '%e'};
% 
% % sort by date
...
% par.Properties.VariableDescriptions = {'', '%e', '%e', '%e', '%e', '%e', '%e', '%e'};
% 
...
if ~isfile(fullfile(ila.instrument.FLOW.path.prod, 'KM24SOPACE_InLine_SUVF6244_20241024_20241216_Product_v20260505.mat'))
  ...
  suvf.Properties.VariableDescriptions = {'', '%e', '%e', '%e', '%e', '%e', '%e', '%e'};
  ...
  load(fullfile(ila.instrument.FLOW.path.prod, 'KM24SOPACE_InLine_SUVF6244_20241024_20241216_Product_v20260505.mat'))
end

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% HBB %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
list_leg = table();
list_leg.dt = [...
  datetime(2024,10,23) datetime(2024,11,26);
  datetime(2024,11,29) datetime(2024,12,16);
  ];
list_leg.experiment = {'PVST_HOTPACE';'PVST_SOPACE';};
list_leg.cruise = {'KM2418';'KM2419';};
...
for i=1:size(list_leg, 1)
  ...
  % add units and precision
  hbb_temp.Properties.VariableNames = {'dt','lat','lon','sst','sss','fdom','VSF124','bbp','VSF124_sd','bincount','gamma_bbp','poc','cphyto_bbp','flag_bit','chl_ap676lh','cp660','bp550'};
  hbb_temp.Properties.VariableUnits = {'','degrees','degrees','degreesC','PSU','v_uncalibrated','1/m/sr','1/m','1/m/sr','none','unitless','ug/L','ug/L','unitless','ug/L','1/m','1/m'};
  % hbb_temp.Properties.VariableDescriptions = {'','%.4f','%.4f','%.4f','%.4f','%.4f','%.4f','%.4f','%.4f','%i','%.4f','%.2f','%.2f','%i'};
  hbb_temp.Properties.VariableDescriptions = {'','%e','%e','%e','%e','%e','%e','%e','%e','%i','%e','%e','%e','%i','%e','%e','%e'};

  % add units and precision
  hbb4seabass = hbb_temp(:, ~contains(hbb_temp.Properties.VariableNames, {'fdom','gamma_bbp','poc','cphyto','flag_bit','chl_ap676lh','cp660','bp550'}));
  ...
  % plot time series
  ila.visProd_timeseries()
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_HBB_bbr_diagnostic']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_HBB_bbr_diagnostic']), 'fig')
  close figure 26
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_HBB_BBparticulate_timeseries']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_HBB_BBparticulate_timeseries']), 'fig')
  close figure 24
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_HBB_POCparticulate_timeseries']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_HBB_POCparticulate_timeseries']), 'fig')
  close figure 23
  % saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_HBB_BBdissolved_timeseries']), 'jpg')
  % saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_HBB_BBdissolved_timeseries']), 'fig')
  % close figure 25
  
  SimpleMap(hbb4seabass.bbp(:,hbb_lambda == 550), hbb4seabass(:,1:3), 'bbp (550 nm) [m^-^1]')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_hbb_bbp_map']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_hbb_bbp_map']), 'fig')
  close figure 1
  
  filename_part = sprintf('%s_InLine_%s_%s_%s_Particulate_v%s.sb', list_leg.cruise{i}, ila.cfg.instruments2run{:}, ...
    datetime(min(hbb4seabass.dt), 'Format', 'yyyyMMdd'), datetime(max(hbb4seabass.dt), 'Format', 'yyyyMMdd'), ...
    datetime('today', 'Format', 'yyyyMMdd'));
  
  % export product to SeaBASS format
  ila.meta.documents = sprintf('%s_HBB_ProcessingReport_v%s.pdf', cruise, datetime(2025,2,20, 'Format', 'yyyyMMdd'));
...
% plot time series
ila.visProd_timeseries()
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_HBB_bbr_diagnostic']), 'jpg')
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_HBB_bbr_diagnostic']), 'fig')
close figure 26
...
SimpleMap(hbb.bbp(:,hbb_lambda == 550), hbb(:,1:3), 'bbp (550 nm) [m^-^1]')
...
list_leg.dt = [...
  ...
  datetime(2024,12,2) datetime(2024,12,16);
  ];

list_leg.experiment = {'PVST_HOTPACE';'PVST_SOPACE';'PVST_SOPACE';};
list_leg.cruise = {'KM2418';'KM2419';'KM2419';};
list_leg.devfile = {'ACS091_20240510.dev';'ACS091_20240510.dev';'ACS111_20231006.dev';};
list_leg.instru = {'ACS91';'ACS91';'ACS111';};
...
for i=1:size(list_leg, 1)
  ...
  ila.visProd_timeseries()
  saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [list_leg.cruise{i} '_' ref '_ACS_prod_regressions']), 'jpg')
  saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [list_leg.cruise{i} '_' ref '_ACS_prod_regressions']), 'fig')
  close figure 14
  saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [list_leg.cruise{i} '_' ref '_ACS_prod_part_timeseries']), 'jpg')
  saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [list_leg.cruise{i} '_' ref '_ACS_prod_part_timeseries']), 'fig')
  close figure 10
  if isfield(ila.instrument.(list_leg.instru{i}).prod, 'g')
    if ~isempty(ila.instrument.(list_leg.instru{i}).prod.g)
      saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [list_leg.cruise{i} '_' ref '_ACS_prod_diw_timeseries']), 'jpg')
      saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [list_leg.cruise{i} '_' ref '_ACS_prod_diw_timeseries']), 'fig')
      close figure 16
    end
  end
  ...
  ACp = movevars(ACp, 'fdom', 'Before','ap');
  ACp.Properties.VariableUnits = {'','degrees','degrees','degreesC','PSU','ppb','1/m','1/m','1/m','1/m',...
    'none','none','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m','1/m', ...
  ...
  ACp.Properties.VariableDescriptions = {'','%e','%e','%e','%e','%e','%e','%e','%e','%e',...
    '%i','%i','%e','%e','%e','%e','%e','%e','%e','%e','%e','%e','%e','%e','%e','%e','%e',...
    '%e','%e','%e','%e','%e','%e','%e','%e','%e','%i'};

  % split into particulate table
  id_ancil = logical(sum(categorical(ACp.Properties.VariableNames) == {'dt','lat','lon','t','s','fdom','ap_n'}'));
  id_particulate = logical(sum(categorical(ACp.Properties.VariableNames) == {'ap','ap_unc','cp','cp_unc'}'));
  ...
  filename_part = sprintf('%s_InLine_%s_Particulate_%s_%s_v%s.sb', list_leg.cruise{i}, ref, ...
    datetime(min(data_AC.particulate.(ref).dt), 'Format', 'yyyyMMdd'),...
    datetime(max(data_AC.particulate.(ref).dt), 'Format', 'yyyyMMdd'),...
    datetime('today', 'Format', 'yyyyMMdd'));
  ila.meta.documents = sprintf('%s_ACS_ProcessingReport_v%s.pdf', cruise, datetime(2025,2,20, 'Format', 'yyyyMMdd'));
  ...
  % include modelled ag/cg
  id_ancil = logical(sum(categorical(ACp.Properties.VariableNames) == {'dt','lat','lon','t','s','fdom','ap_n','cp_n'}'));
  id_particulate = logical(sum(categorical(ACp.Properties.VariableNames) == {'ap','ap_unc','cp','cp_unc','ag_modelled','cg_modelled'}'));
  acs_part = ACp(:, id_ancil | id_particulate);
  % rename variables for .mat
  acs_part = renamevars(acs_part, {'t','s'}, {'sst','sss'});
  save(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, strrep(filename_part, '.sb', '')), 'acs_part', 'lambda');
  % rename variables for .sb
  acs_part.cp_n = [];
  acs_part = renamevars(acs_part, {'fdom','ap_n','ag_modelled','cg_modelled'}, {'cdomf','bincount','ag','cg'});
  acs_part = movevars(acs_part, {'ag','cg'}, 'Before','bincount');
  % export particulate with modelled ag cg to SeaBASS format
  filename_partdiss = strrep(filename_part, '_Particulate_', '_ParticulateDissolved_');
  exportSeaBASS(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, filename_partdiss),...
      ila.meta,...
      acs_part,...
      {'',string(ila.instrument.(list_leg.instru{i}).lambda_a),...
      string(ila.instrument.(list_leg.instru{i}).lambda_c),...
  ...
  if isfield(ila.instrument.(list_leg.instru{i}).prod, 'g')
    if ~isempty(ila.instrument.(list_leg.instru{i}).prod.g)
      ...
      ACg.Properties.VariableDescriptions = {'','%e','%e','%e','%e','%e','%e','%e','%e','%e','%i','%i',...
...
ila.visProd_timeseries()
saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [cruise '_ACS_prod_regressions']), 'jpg')
saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [cruise '_ACS_prod_regressions']), 'fig')
close figure 14
saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [cruise '_ACS_prod_timeseries']), 'jpg')
saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [cruise '_ACS_prod_timeseries']), 'fig')
close figure 10
if isfield(ila.instrument.(list_leg.instru{i}).prod, 'g')
  if ~isempty(ila.instrument.(list_leg.instru{i}).prod.g)
    saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [cruise '_' ref '_ACS_prod_diw_timeseries']), 'jpg')
    saveGraph(fullfile(ila.instrument.(list_leg.instru{i}).path.prod, 'plots', [cruise '_' ref '_ACS_prod_diw_timeseries']), 'fig')
    close figure 16
  end
end
...
filename = sprintf('%s_InLine_ACS_%s_%s_ProductsFull_v%s', cruise, ...
...
list_leg.dt = [...
  datetime(2024,10,23) datetime(2024,11,26);
  datetime(2024,11,28) datetime(2024,12,16);
  ];
list_leg.experiment = {'PVST_HOTPACE';'PVST_SOPACE';};
list_leg.cruise = {'KM2418';'KM2419';};
...
for i=1:size(list_leg, 1)
  ...
  lisst_temp.Properties.VariableDescriptions = {'','%e','%e','%e','%e','%e','%e','%e','%e','%e','%e','%e','%i'};
  ...
  % plot time series
  ila.visProd_timeseries()
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_VSD']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_VSD']), 'fig')
  close figure 103
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_PSD']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_PSD']), 'fig')
  close figure 102
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_VSF']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_VSF']), 'fig')
  close figure 101
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_cp']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_cp']), 'fig')
  close figure 80
  
  SimpleMap(lisst4seabass.cp670, lisst4seabass(:,1:3), 'c_p (670 nm) [m^{-1}]')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_cp_map']), 'jpg')
  saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [list_leg.cruise{i} '_LISST100X_cp_map']), 'fig')
  close figure 1
  ...
  % export product to SeaBASS format
  ila.meta.documents = sprintf('%s_LISST100X_ProcessingReport_v%s.pdf', cruise, datetime(2025,3,1, 'Format', 'yyyyMMdd'));
...
% plot time series
ila.visProd_timeseries()
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_VSD']), 'jpg')
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_VSD']), 'fig')
close figure 103
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_PSD']), 'jpg')
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_PSD']), 'fig')
close figure 102
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_VSF']), 'jpg')
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_VSF']), 'fig')
close figure 101
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_cp']), 'jpg')
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_cp']), 'fig')
close figure 80

SimpleMap(lisst.cp670, lisst(:,1:3), 'c_p (670 nm) [m^{-1}]')
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_cp_map']), 'jpg')
saveGraph(fullfile(ila.instrument.FLOW.path.prod, 'plots', [cruise '_LISST100X_cp_map']), 'fig')
close figure 1
...
par.Properties.VariableUnits = {'', 'degrees', 'degrees', 'E.m^{-2}', 'E.m^{-2}', 'none'};
% par.Properties.VariableDescriptions = {'', '%.4f', '%.4f', '%.4f', '%.4f', '%.2f'};
par.Properties.VariableDescriptions = {'', '%e', '%e', '%e', '%e', '%ef'};
...
par.par = par.par./100; % change from uE.cm^-2 to E.m^-2

SimpleMap(par.par, par(:,1:3), 'PAR [E.m^{-2}]')
