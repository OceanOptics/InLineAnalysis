...
% Atlantic by weeks
% ila.cfg.days2run = datetime(2025,12,16):datetime(2025,12,23);
% ila.cfg.days2run = datetime(2025,12,24):datetime(2026,1,1);
% ila.cfg.days2run = datetime(2026,1,2):datetime(2026,1,9);

% Panama-Majuro 3 days at the time
% ila.cfg.days2run = datetime(2025,12,19):datetime(2025,12,21);
% ila.cfg.days2run = datetime(2025,12,25):datetime(2025,12,27);
% ila.cfg.days2run = datetime(2025,12,28):datetime(2025,12,30);
% ila.cfg.days2run = datetime(2025,12,31):datetime(2026,1,2);
% ila.cfg.days2run = datetime(2026,1,3):datetime(2026,1,5);
% ila.cfg.days2run = datetime(2026,1,6):datetime(2026,1,9);

% Panama-Majuro by weeks
% ila.cfg.days2run = datetime(2026,1,27):datetime(2026,2,9);
% ila.cfg.days2run = datetime(2026,2,10):datetime(2026,2,17);
% ila.cfg.days2run = datetime(2026,2,18):datetime(2026,2,23);
% ila.cfg.days2run = datetime(2026,2,24):datetime(2026,3,3);
% ila.cfg.days2run = datetime(2026,3,4):datetime(2026,3,14);

% Panama-Majuro 3 days at the time
% ila.cfg.days2run = datetime(2026,2,2):datetime(2026,2,4);
% ila.cfg.days2run = datetime(2026,2,5):datetime(2026,2,8);
% ila.cfg.days2run = datetime(2026,2,11):datetime(2026,2,13);
% ila.cfg.days2run = datetime(2026,2,14):datetime(2026,2,16);
% ila.cfg.days2run = datetime(2026,2,17):datetime(2026,2,19);
% ila.cfg.days2run = datetime(2026,2,20):datetime(2026,2,22);
% ila.cfg.days2run = datetime(2026,2,23):datetime(2026,2,25);
% ila.cfg.days2run = datetime(2026,2,26):datetime(2026,2,28);
% ila.cfg.days2run = datetime(2026,2,29):datetime(2026,2,31);
% ila.cfg.days2run = datetime(2026,3,1):datetime(2026,3,3);
% ila.cfg.days2run = datetime(2026,3,4):datetime(2026,3,6);
% ila.cfg.days2run = datetime(2026,3,7):datetime(2026,3,9);
% ila.cfg.days2run = datetime(2026,3,10):datetime(2026,3,12);
% ila.cfg.days2run = datetime(2026,3,11):datetime(2026,3,14);

% Majuro-Japon by weeks
% ila.cfg.days2run = datetime(2026,3,22):datetime(2026,3,31);
% ila.cfg.days2run = datetime(2026,4,1):datetime(2026,4,9);

% Majuro-Japon 3 days at the time
% ila.cfg.days2run = datetime(2026,3,22):datetime(2026,3,24);
% ila.cfg.days2run = datetime(2026,3,25):datetime(2026,3,27);
% ila.cfg.days2run = datetime(2026,3,28):datetime(2026,3,30);
% ila.cfg.days2run = datetime(2026,3,31):datetime(2026,4,2);
% ila.cfg.days2run = datetime(2026,4,3):datetime(2026,4,5);

% by leg
% ila.cfg.days2run = datetime(2025,12,16):datetime(2026,1,9);
% ila.cfg.days2run = datetime(2026,1,27):datetime(2026,3,14);
% ila.cfg.days2run = datetime(2026,3,22):datetime(2026,4,9);

% entire cruise
ila.cfg.days2run = datetime(2025,12,16):datetime(2026,4,9);

%% %%%%%%%%%%%%%% PROCESSING CHRONOLOGY RECOMMENDATIONS: %%%%%%%%%%%%%%% %%
% Instruments available: 'NMEA','FLOW','SBE455462270286','SUVF6266','ACS111','ACS279','HyperBB8005','LISST100X1183','QCR2150A50351'
%%% Run ReadRaw 2 weeks by 2 weeks without going further (reloading 'ila' structure between each run to clear memory)
% ila.cfg.instruments2run = {'FLOW','NMEA','SUVF6266','SBE455462270286','LISST100X1183','HyperBB8005','ACS279'};
%%% Process to the end
% ila.cfg.instruments2run = {'FLOW','NMEA','SBE455462270286'};
...
% ila.cfg.instruments2run = {'FLOW','ACS279'}; % 3 days at the time
...
% ila.cfg.instruments2run = {'FLOW','ACS279','SUVF6266','SBE455462270286'};
...
% ila.instrument.SBE455462270286.prod.a = [ila.instrument.SBE455462270286.prod.a; ila.instrument.SBE384504970286.prod.a];
% ila.instrument.SBE455462270286.prod.a = sortrows(ila.instrument.SBE455462270286.prod.a, 'dt');
% ila.instrument.SBE384504970286.prod.a = ila.instrument.SBE455462270286.prod.a;
...
ila.SpectralQC('LISST',{'raw'}, false, {'fsw','all'});
